...
### Fixed
- **OOM on large projects**: Pass ignore patterns to Rust-side `scan_dir` so directories like `target/`, `node_modules/`, `.git/` are skipped during the filesystem walk, before any Lua tables are created
- Removed redundant Lua-side filtering of ignored directories (now handled by Rust)
